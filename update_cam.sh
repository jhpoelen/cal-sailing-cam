...
update_trendnet_tv_ip110w_a() {
	...
	echo `date` update cam [$2] starting...
        curl -u admin:admin --globoff "http://$1/cgi/mjpg/mjpeg.cgi" | avconv -i pipe:0 -frames:v 1 pipe:1 | curl -F $2=@- -v $UPLOAD_URL
        curl -u admin:admin --globoff "http://$1/admin/ultracam.jar" | curl -F "ultracam.jar"=@- -v $UPLOAD_URL
	echo `date` update cam [$2] done.
}
